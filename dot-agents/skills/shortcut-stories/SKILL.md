...
- **Code-style refactors never become stories.** Skip PRs that only reshape code without changing what anyone can do or see, however many of them there are. Examples are inlining single-use types, replacing indexed access types, renames, and formatting. Removing a retired feature, table, or endpoint is different: it retires something teammates knew about, so it can be a story.
